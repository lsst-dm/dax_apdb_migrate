...
This is relevant for Postgres only, SQLite does not know about time zones, so the script is no-op for SQLite.
...
Additionally ``felis`` package must be setup before running this script.
